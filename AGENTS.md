...
- `.env.example` shows the schema: `BOT_TOKEN`, `USERS`, `DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL`, `DEEPSEEK_BASE_URL`, `DATA_DIR`, `QDRANT_URL`, `EMBEDDING_MODEL`, the optional email vars (`EMAIL_ENABLED`, `EMAIL_POLL_SECONDS`, `EMAIL_QUERY_DAYS`, `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN`, `GMAIL_USER`), and the optional calendar vars (`CALENDAR_ENABLED`, `GOOGLE_CALENDAR_ID`, `CALENDAR_TIMEZONE`).
...
- **Calendar scheduling** (`CALENDAR_ENABLED`) lets the bot create Google Calendar appointments from chat. It reuses the same Google OAuth creds as email (`GMAIL_*`; the `calendar.events` scope is already in `GOOGLE_SCOPES`) and requires `DEEPSEEK_API_KEY` (the LLM parses the natural-language date). Events are created without reminders. `GOOGLE_CALENDAR_ID` (default `primary`) and `CALENDAR_TIMEZONE` (default `America/Bogota`) tune it.
...
- **LLM**: LangChain `ChatOpenAI` via DeepSeek-compatible API — `DeepseekLlm` (implements `Llm`) in `src/infrastructure/llm/deepseekLlm.ts`. Three more DeepSeek-backed adapters live alongside it: `LlmTitler` (`llmTitler.ts`, implements `Titler`), `LlmTagger` (`llmTagger.ts`, implements `Tagger`) and `LlmEmailSummarizer` (`llmEmailSummarizer.ts`, implements `EmailNoteSummarizer`) — all optional, only constructed when `DEEPSEEK_API_KEY` is set. The titler/tagger auto-name and auto-tag documents/notes; the summarizer triages forwarded email bodies (decide whether to keep + rewrite into a concise note).
