...
| Start / deploy containers | `./start.sh` — `git pull` + `sudo docker compose up -d --build` (full image rebuild) |
...
- `.env.example` shows the schema: `BOT_TOKEN`, `USERS`, `LLM_API_KEY`, `LLM_MODEL`, `LLM_BASE_URL`, `DATA_DIR`, `QDRANT_URL`, `EMBEDDING_MODEL`, the optional email vars (`EMAIL_ENABLED`, `EMAIL_POLL_SECONDS`, `EMAIL_QUERY_DAYS`, `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN`, `GMAIL_USER`), and the optional calendar vars (`CALENDAR_ENABLED`, `GOOGLE_CALENDAR_ID`, `CALENDAR_TIMEZONE`).
...
- `LLM_API_KEY` is optional — without it text questions return an error but file upload/indexing still work. `LLM_MODEL` defaults to `openai/gpt-4o`; `LLM_BASE_URL` defaults to `https://openrouter.ai/api/v1`.
- `WEB_PORT`, `WEB_HOST`, `WEB_PASSWORD` configure the web UI for file upload (always started; `WEB_PORT` defaults to `3000`).
- **Calendar scheduling** (`CALENDAR_ENABLED`) lets the bot create Google Calendar appointments from chat. It reuses the same Google OAuth creds as email (`GMAIL_*`; the `calendar.events` scope is already in `GOOGLE_SCOPES`) and requires `LLM_API_KEY` (the LLM parses the natural-language date). Events are created without reminders. `GOOGLE_CALENDAR_ID` (default `primary`) and `CALENDAR_TIMEZONE` (default `America/Bogota`) tune it.
...
- **`src/domain/`** — pure core, no external deps. `types.ts` (`FileRecord`, `PendingPassword`, `Note`, `ChunkMetadata`, `SearchResult`, `Session`, `ConversationMessage`, `IncomingEmail`, `EmailAttachment`, `CalendarEvent`) and `ports.ts` (interfaces: `DocumentRepository`, `TextExtractor`, `ArchiveExtractor`, `Ocr`, `Chunker`, `Embedder`, `VectorIndex`, `PasswordVault`, `NoteRepository`, `SessionStore`, `Titler`, `Tagger`, `Tool`, `Llm`, `EmailSource`, `ProcessedEmailLog`, `EmailNoteSummarizer`, `CalendarService`). Plus four pure, zero-dependency helper modules shared across layers: `fileType.ts` (magic-byte `isPdfBuffer`/`isImageBuffer`/`isZipBuffer` routing for `application/octet-stream` attachments), `icalendar.ts` (`isIcsBuffer` + `parseIcsEvents` — parses `.ics` appointment invites into calendar events, skipping cancellations), `tags.ts` (`normalizeTags` — lowercases, dedupes, ISO-normalizes dates, caps at `MAX_TAGS`), and `date.ts` (`toIsoDate` — canonicalizes dates to `YYYY-MM-DD`).
...
- **LLM**: LangChain `ChatOpenAI` via an OpenAI-compatible API (OpenRouter by default) — `ChatLlm` (implements `Llm`) in `src/infrastructure/llm/chatLlm.ts`. `chatModel.ts` centralizes client construction for it and the three supporting adapters: `LlmTitler` (`llmTitler.ts`, implements `Titler`), `LlmTagger` (`llmTagger.ts`, implements `Tagger`) and `LlmEmailSummarizer` (`llmEmailSummarizer.ts`, implements `EmailNoteSummarizer`). All are optional and only constructed when `LLM_API_KEY` is set. The titler/tagger auto-name and auto-tag documents/notes; the summarizer triages forwarded email bodies (decide whether to keep + rewrite into a concise note).
...
- **Archive extraction**: `AdmZipExtractor` (implements `ArchiveExtractor`) in `src/infrastructure/archive/admZipExtractor.ts` — uses `adm-zip` to expand a zip in memory into its file entries. `IngestEmail` uses it to unpack forwarded zip attachments before routing each inner file.
...
- **Calendar scheduling (Google Calendar)**: optional, only wired when `CALENDAR_ENABLED` + Google creds are present. `GoogleCalendarService` (`src/infrastructure/google/googleCalendarService.ts`, implements `CalendarService`) creates events via the Calendar API, reusing the **shared** OAuth client (`createGoogleAuth`, hoisted in `main.ts` so email and calendar share one consent). Events are inserted with no reminders (`reminders.overrides: []`). The scheduling entry point is the `schedule_appointment` tool inside `AskQuestion` (agentic RAG): the LLM parses the natural-language date (with "today" + `CALENDAR_TIMEZONE` anchored into the system prompt) into an ISO 8601 `startIso`, and the tool creates the event, prefixing the patient's name (from the `USERS` registry) for attribution in the shared calendar. The tool is only registered when `AskQuestion` receives a non-null `CalendarService`.
...
- **No build step for the backend.** Bun runs TypeScript directly, so `src/` and `web/dist/` are bind-mounted into the `app` container. Deploys always go through `./start.sh` (`git pull` + `docker compose up -d --build`), which rebuilds the image and restarts. `node_modules` stays baked in the image (not mounted).
...
| Send text | Button labels first (**Subir**, **Archivos**, **Contraseña**, **Nota**, **Notas**, **Nuevo**), then pending states (password add, note, PDF password), otherwise an `AskQuestion` (RAG) call (requires `LLM_API_KEY`) |
| Agendar cita | Within an `AskQuestion` turn: if the user asks to schedule ("agenda una cita el 12 de julio a las 3pm") and calendar is enabled, the LLM calls the `schedule_appointment` tool → creates the Google Calendar event (no reminders) and confirms. If no time is given, the bot asks for it first. |
...
- `AskQuestion` (RAG) is only constructed when `LLM_API_KEY` is set; it's passed as `null` to `BotApp` otherwise, so guard for that in handlers.
...
- **Ingest step** (`IngestEmail.run`), per email:
  ...
  3. **Body → Note (LLM-triaged)** — the body is first run through `EmailNoteSummarizer` (`LlmEmailSummarizer`, OpenRouter): only bodies with useful health info (cita confirmada/cancelada/reprogramada, resultados disponibles, novedad de un trámite) become a Note, rewritten into a short clear Spanish summary; noise (publicidad, boletines, notificaciones automáticas) is dropped (no note). Without an LLM configured, the raw `subject + body` is saved as before. The summary is indexed via `IndexNote` so RAG can retrieve it.
  4. **Attachments → indexable-only, zips expanded, .ics → calendar** — each attachment is routed by MIME with a magic-bytes fallback (email attachments are often `application/octet-stream`). Calendar invites (`.ics` / `text/calendar` / `application/ics`, or a `BEGIN:VCALENDAR` sniff) are parsed via `icalendar.ts` and turned into Google Calendar events (attributed with the patient's name, cancellations skipped) — the `.ics` file itself is **never stored or indexed**; when calendar is disabled it's simply dropped. Zip attachments (`application/zip`/`x-zip-compressed` or `PK` magic bytes) are first expanded via `ArchiveExtractor` (`AdmZipExtractor`) and their inner files routed the same way. Only indexable files are kept: PDF → `IndexPdf`, image → `IndexImage` (both content-deduped via `repo.findByContent`). Anything that cannot be indexed is **dropped, not stored** — better to discard than to archive an unsearchable file.
